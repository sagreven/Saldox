...
class SofarModbusClient:
            ...
            try:
                if reg.fc == "input":
                    resp = await self._client.read_input_registers(
                        address=reg.address, count=reg.word_count, slave=self._unit_id
                    )
                else:
    ...
    async def write_holding(self, reg: Register, value: int) -> None:
        ...
        assert self._client is not None
        if reg.word_count == 1:
            resp = await self._client.write_register(address=reg.address, value=value & 0xFFFF, slave=self._unit_id)
        else:
            hi = (value >> 16) & 0xFFFF
            lo = value & 0xFFFF
            resp = await self._client.write_registers(address=reg.address, values=[hi, lo], slave=self._unit_id)
        if resp.isError():
